  ...
  <div>
    ...
    >
      <el-form style="width: 80%" :model="tmForm" :rules="rules" ref="tmForm">
        <!-- el-form代表的是表单，表单里面是表单项
            每个表单项都可以通过label设置表单项名称
            每个表单项都可以通过label-width设置表单项名称的宽度
        form当中都会写 :model="对象" 作用：1、以后用来去对这个form进行表单验证 2、用来标识这个form收集的数据收集到哪
        -->
        ...
        <el-form-item label="品牌LOGO" label-width="100px" prop="logoUrl">
          <!-- 在拷贝upload组件的时候，把html、css、js相关的东西全部拷贝 -->
          <!-- action 控制的是上传的接口地址 -->
          <!-- 上传图片分为两步：
          ...
          <!--
            ...
            如果添加了，则往这发http://localhost:9528/dev-api/admin/product/fileUpload  就会被代理转发到真实的地址
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        rules: {
          // element-ui 提供的表单验证规则，它是一个对象
          // 而每个要验证的字段规则都是一个数组
          // 数组里面是对象，每一个对象就代表是验证的一个规则
          // 每个规则对象里面包含三个东西：1、规则 2、错误提示信息 3、触发时机（验证时机）
          // 触发时机：一共有三个，1.失去焦点的时候blur  2.内容改变的时候change  3.整体验证的时候
          tmName: [
            { required: true, message: '请输入品牌名称', trigger: 'blur' },
            // { min: 2, max: 10, message: '长度在 2 到 10 个字符', trigger: 'change' }, // 内置验证规则
    ...
    methods: {
      ...
      showUpdateDialog(row) {
        ...
        // 深浅拷贝
        // 拷贝？
        // var a = [1,2,3]  var b = a   是不是拷贝？ 答：不是拷贝
        // 什么是拷贝：必须出现另外一个内存空间，才叫拷贝。
        // 深浅拷贝：说的是拷贝过程中数据的类型问题。
        // 深浅拷贝面向的数据必然是对象格式，基本数据类型不存在深浅拷贝。
        ...
        // 这里的row当中只有基本数据类型，所以说浅拷贝就ok，没必要深拷贝，当然深拷贝也是可以的
