        ...
        <el-table border style="width: 100%" :data="spuForm.spuSaleAttrList">
          ...
          <el-table-column prop="prop" label="属性值名称列表" width="width">
            <template slot-scope="{ row, $index }">
              ...
              inputValue必须是用户输入了数据，row身上才有这个属性
  ...
  export default {
    ...
    methods: {
      ...
      handlePictureSuccess(res, file, fileList) {
        // 上传成功，也得把最后的图片列表收集起来
        // 特别注意：新上传的图片和老图片的内部属性是不一样的
        //    新图片内部没有imgName和imgUrl，但是有name和url，只是url是本地地址，是错的。新图片真正的url存在它的response当中
        //    老图片内部既有imgName和imgUrl，也有name和url，都是对的
        // 注意后期发请求服务器所需要的图片属性格式
      ...
      handlePictureCardPreview(file) {
        this.dialogImageUrl = file.url // 把需要放大的图片src赋值到对话框当中的图片中
      ...
      async getUpdateSpuFormInitData(spu, category3Id) {
        ...
        if (imageResult.code === 200) {
          ...
          // 为了让upload可以展示图片列表，必须把请求回来的图片当中添加上name和url属性
      ...
      addSaleAttr() {
        ...
        this.spuForm.spuSaleAttrList.push(obj) // 把销售属性对象添加到spu的销售属性列表当中
      ...
      handleInputConfirm(row) {
        ...
        // 之前平台属性要去除去自身，是因为之前我们平台属性用的是占位思想，还没有输入平台属性值名称，就已经添加了这个平台属性值，因此要除去自身
      ...
      async save() {
        ...
        //   imgUrl: "string",
        // },
        // 目前我们的图片列表数据结构，包含之前已经有的和自己最近上传的
        // 之前已经有的图片内部是有imgName和imgUrl，也有name和url。而自己上传的只有name和url（错的）
