  ...
  export default {
    ...
    data() {
      return {
        ...
        isShowList: true, // 这个数据是为了完成三级分类的可操作性传递过去的，和这里的三个页面显示隐藏没关系
    ...
    methods: {
      ...
      // 点击修改spu的回调，显示修改Spu的页面（其实和添加是同一个页面）
      ...
      backSuccess() {
        // 重新发请求获取spu列表数据
        // 返回列表页要区分：添加成功返回的还是修改成功返回的，因为请求哪一页的数据是不一样的
        if (this.flag) {
      ...
      async showSkuList(row) {
        ...
        this.loading = true // 数据请求回来之前开启loading加载动效
        const result = await this.$API.sku.getListBySpuId(row.id)
        if (result.code === 200) {
          this.skuList = result.data
        }
        this.loading = false // 数据请求回来之后关闭loading加载动效
      },
