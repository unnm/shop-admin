  ...
  <div>
    ...
    <el-card style="margin-top: 20px">
      ...
      <div v-show="!isShowList">
        ...
        当要收集属性值的时候，用了占位的思想，先往数组里面添加一个属性值对象，只不过属性值的名称此时是空的
        但是，一旦添加这个属性值对象，表格当中就会出现一行，我们可以使用输入框再去给刚才添加的属性值对象，收集名称
        -->
        <el-table
          border
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        attrForm: {
          ...
          // 以后都要记住，data当中不能使用this来获取data当中的另外一个数据，因为data数据是一个对象，初始化的时候，谁先谁后不一定
          categoryId: 0,
          categoryLevel: 3
          // id:0
        }
    ...
    methods: {
      ...
      showUpdateDiv(row) {
        this.isShowList = false
        // this.attrForm = {...row} // 这次这里使用浅拷贝没用，因为内部数组是一个对象类型，浅拷贝拷的是地址，数组用的还是同一个
        this.attrForm = cloneDeep(row) // 利用lodash包提供的cloneDeep函数实现对象深拷贝
        ...
        // 但是如果不是一开始就有的属性，就不会做监视，既然没做监视，后期添加的这个属性就不是响应式属性
      ...
      toLook(row) {
        // 1、失去焦点的时候需要判定数据中是否有属性值名称，如果没有值或者值不合法，不能切换为查看模式
        if (row.valueName.trim() === '') {
          row.valueName = ''
          return
        }
        // 2、我们还得判断当前输入的这个属性值名称，在除去自身以外，其余的属性值当中是否存在，如果存在就重复了，不能切换为查看模式
