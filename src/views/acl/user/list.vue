...
<script>
  ...
  export default {
    ...
    // 我们一般都是在mounted去发请求获取数据，但是也可以在created中去发
    // 推荐在mounted中去发请求
    created() {
      this.getUsers()
    },
    ...
    methods: {
      ...
      async getRoles() {
        ...
        this.checkAll = allRolesList.length === assignRoles.length // 复选框的全选状态
        this.isIndeterminate =
          assignRoles.length > 0 && assignRoles.length < allRolesList.length // 复选框的不确定状态（不确定选了几个）
      },
